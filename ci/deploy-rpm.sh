...
VUL_VERSION=$(find dist/ -type f -name "*64bit.rpm" -printf "%f\n" | head -n1 | sed -nre 's/^[^0-9]*(([0-9]+\.)*[0-9]+).*/\1/p')
...
function create_common_rpm_repo () {
        ...
        for arch in ${ARCHES[@]}; do
                ...
                createrepo_c -u https://github.com/khulnasoft-lab/vul/releases/download/ --location-prefix="v"$VUL_VERSION --update $rpm_path/$arch
...
function create_rpm_repo () {
        ...
        createrepo_c -u https://github.com/khulnasoft-lab/vul/releases/download/ --location-prefix="v"$VUL_VERSION --update $rpm_path

        rm ${rpm_path}/*64bit.rpm
}

echo "Create RPM releases for Vul v$VUL_VERSION"
...
git add .
git commit -m "Update rpm packages for Vul v$VUL_VERSION"

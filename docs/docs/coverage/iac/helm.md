...
```bash
vul conf --helm-set securityContext.runAsUser=0 ./charts/mySql
```
...
```bash
vul conf --helm-values overrides.yaml ./charts/mySql
``` 
...
```bash
vul config --helm-set-string name=false ./infrastructure/tf
```
...
```bash
vul conf --helm-set-file environment=dev.values.yaml ./charts/mySql
```

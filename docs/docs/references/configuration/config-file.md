...
# Same as '--report' (available with 'vul k8s')
# Default is all
report: all
...
# Same as '--ignorefile'
# Default is '.vulignore'
ignorefile: .vulignore
...
db:
  ...
  # Default is 'ghcr.io/khulnasoft-lab/vul-db'
  repository: ghcr.io/khulnasoft-lab/vul-db

  # Same as '--java-db-repository'
  # Default is 'ghcr.io/khulnasoft-lab/vul-java-db'
  java-repository: ghcr.io/khulnasoft-lab/vul-java-db
```
...
```yaml
image:
  # Same as '--input' (available with 'vul image')
  # Default is empty
  input:
...
secret:
  ...
  # Default is 'vul-secret.yaml'
  config: config/vul/secret.yaml
```
...
## Repository Options
Available with git repository scanning (`vul repo`)
...
Available for cloud scanning (currently only `vul aws`)
...
[example]: https://github.com/khulnasoft-lab/vul/tree/{{ git.tag }}/examples/vul-conf/vul.yaml

...
```bash
$ vul image --scanners config IMAGE_NAME
```

```bash
$ vul fs --scanners config /path/to/dir
```
...
!!! example
    ...
    $ vul fs --scanners vuln,config,secret --severity HIGH,CRITICAL myapp/
...
$ vul conf --severity HIGH,CRITICAL ./iac
```
...
```bash
cd examplex/misconf/
vul conf --policy custom-policy/policy --policy combine/policy --policy policy.rego --namespaces user misconf/mixed
```

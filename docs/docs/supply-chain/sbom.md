...
```
$ vul image --format spdx-json --output result.json alpine:3.15
```


```
$ vul fs --format cyclonedx --output result.json /app/myproject
```
...
```
$ vul image --format cyclonedx --output result.json alpine:3.15
...
```
$ vul image --scanners vuln --format cyclonedx --output result.json alpine:3.15
```
...
```
$ vul image --format spdx --output result.spdx alpine:3.15
```
...
```
$ vul image --format spdx-json --output result.spdx.json alpine:3.15
```

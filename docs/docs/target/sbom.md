# SBOM scanning
Vul can take the following SBOM formats as an input and scan for vulnerabilities.
...
!!! note
    Passing SBOMs generated by tool other than Vul may result in inaccurate detection 
    because Vul relies on custom properties in SBOM for accurate scanning.

## CycloneDX
Vul supports CycloneDX as an input.
...
## SPDX
Vul supports the SPDX SBOM as an input.
...
In the following example, [Cosign](https://github.com/sigstore/cosign) gets an attestation and Vul scans it.

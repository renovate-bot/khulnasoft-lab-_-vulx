...
```bash
$ vul vm --scanners vuln disk.vmdk
```
...
```shell
$ vul vm --scanners vuln ami:ami-0123456789abcdefg
```

If you want to scan a AMI of non-default setting region, you can set any region via `--aws-region` option.

```shell
$ vul vm --aws-region ap-northeast-1 ami:ami-0123456789abcdefg
```
...
```shell
$ vul vm --scanners vuln ebs:snap-0123456789abcdefg
```
...
```shell
$ vul vm --aws-region ap-northeast-1 ebs:ebs-0123456789abcdefg
```
...
```
$ vul vm --scanners config [YOUR_VM_IMAGE]
```
...
```shell
$ vul vm --scanners license [YOUR_VM_IMAGE]
```

...
func setupServer(addr, token, tokenHeader, cacheDir string) []string {
	osArgs := []string{"vul", "server", "--skip-update", "--cache-dir", cacheDir, "--listen", addr}
	if token != "" {
...
func setupClient(t *testing.T, ignoreUnfixed bool, severity, ignoreIDs []string,
	...
	t.Helper()
	osArgs := []string{"vul", "client", "--cache-dir", cacheDir,
	...
	if len(ignoreIDs) != 0 {
		...
		err = ioutil.WriteFile(vulIgnore, []byte(strings.Join(ignoreIDs, "\n")), 0444)

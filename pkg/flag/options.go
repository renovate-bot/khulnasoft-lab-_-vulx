...
func bind(cmd *cobra.Command, flag *Flag) error {
	if flag == nil {
		...
		// This flag is available only in vul.yaml
...
func bindEnv(flag *Flag) error {
	...
	envName := strings.ToUpper("vul_" + strings.ReplaceAll(flag.Name, "-", "_"))
	...
	for _, alias := range flag.Aliases {
		envAlias := strings.ToUpper("vul_" + strings.ReplaceAll(alias.Name, "-", "_"))
...
func getValue(flag *Flag) any {
	...
	// First, looks for aliases in config file (vul.yaml).
